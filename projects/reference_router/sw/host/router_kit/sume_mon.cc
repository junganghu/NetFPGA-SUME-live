...
sume_mon::clear_dst_filter_rtable()
{
    for(size_t i = 0; i < FIXME_DST_FILTER_MAX; ++i){
    	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, 0);
  	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, SUME_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_ADDRESS | i);
  	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 1);
    }
}
//-----------------------------------------------------------------------------
...
sume_mon::clear_hw_rtable()
{
    for(size_t i = 0; i < FIXME_RT_MAX; ++i){
    	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_HI, 0);
  	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_HI, 0xffffffff);
  	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_LOW, 0);
  	writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, 0);
...
sume_mon::nf_set_mac(const uint8_t* addr, int index)
{
    ...
    {
        case 0:
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_0_HI, mac_hi);
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_0_LOW, mac_lo);
            break;

        case 1:
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_1_HI, mac_hi);
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_1_LOW, mac_lo);
            break;

        case 2:
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_2_HI, mac_hi);
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_2_LOW, mac_lo);
            break;
        case 3:
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_3_HI, mac_hi);
            writeReg(sume, SUME_OUTPUT_PORT_LOOKUP_0_MAC_3_LOW, mac_lo);
            break;
        default:
...
sume_mon::update_interface_table(const iflist& newlist)
{
    ...
    for(; i < newlist.size(); ++i){
        ...
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, htonl(newlist[i].ip.addr));
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, SUME_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_ADDRESS | i);
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 1);
    ...
    writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, htonl(ospf.addr));
    writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, SUME_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_ADDRESS | i);
    writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 1);
}
//-----------------------------------------------------------------------------
...
sume_mon::update_routing_table(const rtable& newrt)
{
    ...
    size_t i = 0;
    for(; i < newrt.size(); ++i){
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_HI, htonl(newrt[i].dest.addr));
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_HI, htonl(newrt[i].mask.addr));
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_LOW, htonl(newrt[i].gw.addr));
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, devtoport[newrt[i].dev]);
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, SUME_OUTPUT_PORT_LOOKUP_0_MEM_IP_LPM_TCAM_ADDRESS | i);
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 1);
    }
...
sume_mon::update_arp_table(const arptable& newat)
{
    ...
    for(; i < newat.size(); ++i){
        ...
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, ntohl(newat[i].ip.addr));
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_HI,  newat[i].etha.octet[0] << 8 | newat[i].etha.octet[1]);
        writeReg(sume,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_LOW, newat[i].etha.octet[2] << 24 |
