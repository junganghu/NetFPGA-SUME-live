...
void write_rtable_to_hw(router_state* rs) {
	...
	for (i = 0; i < SUME_OUTPUT_PORT_LOOKUP_0_MEM_IP_LPM_TCAM_WIDTH; ++i) {
		...
		if (cur) {
			...
			/* write the ip */
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_HI, ntohl(entry->ip.s_addr));
			/* write the mask */
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_HI, ntohl(entry->mask.s_addr));
			/* write the next hop */
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_LOW, ntohl(entry->gw.s_addr));
			/* write the port */
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, getOneHotPortNumber(entry->iface));
			...
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 0x1);
			...
			/* write the mask */
			writeReg(rs->netfpga_regs,SUME_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_HI, 0);
			/* write the next hop */
